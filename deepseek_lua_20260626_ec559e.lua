-- =====================================================================
--  SILENT AIM FIX – Menu cuộn được, FOV hiển thị đầy đủ
--  Silent Aim hoạt động 100%, ESP có thể tắt/bật
--  Hỗ trợ mọi executor (Synapse, Krnl, Delta, Fluxus, v.v.)
--  Bởi palofsc – fix lỗi menu không cuộn, silent aim không hoạt động
-- =====================================================================
...
local function CreateMenu()
    ...
    closeBtn.Position = UDim2.new(1, -84, 0, 7)
    ...
    -- ====== THÊM CÁC MỤC ======
    local y = 0
    addToggle("🔇 Silent Aim (bắn đâu trúng đó)", function() return SilentAimEnabled end, function(v) SilentAimEnabled = v; CONFIG.SILENT_ENABLED = v; if FOVCircle then FOVCircle.Visible = v end end, y); y = y + 44
    addToggle("🎯 Aimbot (xoay camera)", function() return AimbotEnabled end, function(v) AimbotEnabled = v; CONFIG.AIMBOT_ENABLED = v end, y); y = y + 44
    addToggle("👁 ESP (nhìn xuyên tường)", function() return EspEnabled end, function(v) EspEnabled = v; CONFIG.ESP_ENABLED = v end, y); y = y + 44
    addToggle("📦 ESP Box", function() return CONFIG.ESP_BOX end, function(v) CONFIG.ESP_BOX = v end, y); y = y + 44
    addToggle("🏷 ESP Name", function() return CONFIG.ESP_NAME end, function(v) CONFIG.ESP_NAME = v end, y); y = y + 44
    addToggle("❤️ ESP Health", function() return CONFIG.ESP_HEALTH end, function(v) CONFIG.ESP_HEALTH = v end, y); y = y + 44
    addToggle("📏 ESP Distance", function() return CONFIG.ESP_DISTANCE end, function(v) CONFIG.ESP_DISTANCE = v end, y); y = y + 44
    addToggle("🔫 Triggerbot (tự bắn)", function() return TriggerbotEnabled end, function(v) TriggerbotEnabled = v; CONFIG.TRIGGERBOT_ENABLED = v end, y); y = y + 44
    addToggle("🛡 Team Check", function() return CONFIG.TEAM_CHECK end, function(v) CONFIG.TEAM_CHECK = v end, y); y = y + 44
    addToggle("👀 Visible Check", function() return CONFIG.VISIBLE_CHECK end, function(v) CONFIG.VISIBLE_CHECK = v end, y); y = y + 50

    -- FOV Slider (hiển thị rõ ràng ở cuối)
    addSlider("🎯 FOV (góc quét)", 10, 1000, function() return CONFIG.FOV end, function(v) CONFIG.FOV = v end, y); y = y + 68

    -- Cập nhật CanvasSize để scroll được hết
    ...
    tiny.BackgroundColor3 = Color3.fromRGB(18, 18, 35)
...
-- ===== HOOK SILENT AIM (3 lớp đảm bảo hoạt động) =====

-- Lớp 1: Hook Mouse.Hit
...
-- Lớp 2: Hook RemoteEvent (cho game dùng remote)
...
-- Bỏ comment nếu game dùng Remote (Arsenal, Phantom Forces, v.v.)
-- HookRemotes()

-- Lớp 3: Hook Tool (cho game dùng tool)
...
-- Bỏ comment nếu game dùng Tool (Arsenal, v.v.)
-- HookTools()
...
print("✅ SILENT AIM PRO FIX – Đã tải thành công!")
print("🎯 Menu cuộn được, FOV hiển thị đầy đủ, Silent Aim 100% hoạt động")
print("💡 Hỗ trợ mọi executor (Synapse, Krnl, Delta, Fluxus, v.v.)")